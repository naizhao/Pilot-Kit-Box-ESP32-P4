# Expansion Board V3.10 Design (Archived)
...
> V3.10 is the final maintenance design with the QPL9547, comparator, detector,
> decoupling, reset, and USB corrections. The GNSS RF/bias topology was verified
> correct and must not receive a Gate-swap rework. BNO085 remains at 0° on V3.10; firmware must use
> the V3 coordinate transform instead of V4's 90° transform.
> V3.10 also transfers the V4.0 physical-board IFA result: a 50.0mm copper outer
> envelope (48.5mm radiating-arm centerline span), straight-through π network.
...
|---|---|---|
| [`PINMAP.md`](PINMAP.md) | Authoritative pin and net map for the V3.10 design | EN / [中文](PINMAP-zh_CN.md) |
...
> V3.10 是包含 QPL9547、比较器、检波、去耦、复位和 USB 修正的最终维护设计。
> GNSS RF/偏置拓扑已复核为正确，禁止交换 PMOS Gate。
> V3.10 的 BNO085 保持 0°，固件必须使用 V3 坐标变换，不能套用 V4 的 90° 变换。
> V3.10 同步回灌 V4.0 实板 IFA 定型结果：铜箔外包络50.0mm（辐射臂中心线跨度48.5mm），π网络直通。
> 已经打样的 V3.2
> 仍可用于固件调试与天线研究，其 GNSS 选择 Gate 接线也必须保持不变。

| 文档 | 内容 | 语言 |
|---|---|---|
| [`PINMAP.md`](PINMAP.md) | V3.10 设计权威引脚/网络映射 | [EN](PINMAP.md) / 中文 |
