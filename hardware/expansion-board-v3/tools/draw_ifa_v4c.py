#!/usr/bin/env python3
"""v4 IFA 天线预研图 —— 标准 IFA 拓扑（短路腿直接入 GND）。
...
本图仍是预研图，不代表实板已验证。v4生成器、封装库、PCB内嵌ANT1、π网络、J7和
两段射频网络均已同步；普通DRC没有天线相关错误或断路，仍待装盒VNA与1090报文A/B。
"""
import os
import sys
...
# 保持当前 v4 核心图形，绘制已落板的冻结预研尺寸；是否可量产仍由实板测试决定。
LEG = 6.0
ARM_FINAL = 50.2
ARM_DRAW = 52.0
...
S, M = 13, 64
BG = (28, 32, 38); CU = (198, 108, 58); CU_CUT = (150, 60, 40)
...
# 天线
st(SHORT_X, ARM_Y, OPEN_X - CUT, ARM_Y, W, CU)
st(OPEN_X - CUT, ARM_Y, OPEN_X, ARM_Y, W, CU_CUT)
for _t in range(7):
    _x = OPEN_X - CUT + 0.15 + _t * (CUT - 0.3) / 7
    d.line([px(_x, ARM_Y - W / 2), px(_x + 0.45, ARM_Y + W / 2)], fill=(235, 150, 130), width=1)
...
dim_h(SHORT_X, OPEN_X, ARM_Y - CLEAR - 1.2, f"主臂中心线 {ARM_DRAW:.2f}mm（画长待切，切 {CUT:.1f}mm 到 {ARM_FINAL:.2f}）")
...
d.text(px(OPEN_X - 8.5, ARM_Y + 1.5), "开路端（向右；从这头切）", font=FS, fill=(240, 150, 120))
...
y0 = IMG_H - 74
d.text((M, y0 - 28), "v4 板载 1090MHz IFA 预研 — 52.0mm画长已落板，仍待实板验证", font=FB, fill=TXT)
for i, t in enumerate([
    f"六层rev2：历史D=5三点目标50.187mm；实际D=4.988/H=6锚点修正50.230mm（工程仍取{ARM_FINAL:.1f}mm）",
    f"V1装盒实测下移8~20MHz；裸板目标约1104MHz → 工程值 {ARM_FINAL:.1f}mm；画 {ARM_DRAW:.1f}mm 留 {CUT:.1f}mm 可切量",
    f"校准后局部斜率19.83MHz/mm（0.1mm≈1.98MHz）。匹配起点：ZP1 DNP / ZS1 3.6nH / ZP2 3.3pF，并备3.6/3.9pF",
    f"v4生成器/封装库/PCB均为52.0/53.5mm；ANT1左移6.3mm后到H2孔边17.728mm；两天线网络已布且定向DRC/断路为0",
]):
    d.text((M, y0 + i * 20), t, font=FS, fill=(170, 180, 195))

os.makedirs(os.path.dirname(OUT) or ".", exist_ok=True)
img.save(OUT)
print(f"saved: {OUT} {img.size}   主臂画 {ARM_DRAW:.2f} 切后 {ARM_FINAL:.2f}  腿 {LEG:.0f}  D {D:.1f}")
