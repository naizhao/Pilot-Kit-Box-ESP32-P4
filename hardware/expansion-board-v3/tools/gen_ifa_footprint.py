...
⚠️ **当前实现状态（2026-09-04）**：V4.0 实板从开路端**逐刀**切短，
53.5→51.5→51.0→**50.0mm 停刀**。50.0mm 铜箔外包络在装盒+电池铺地下为
1082.5MHz、SWR 1.09、45+j0.5Ω。该实测值取代原先 52.0mm 中心线画长/
53.5mm外包络的预研值。本生成器固定输出48.5mm主臂中心线跨度、50.0mm
铜箔外包络，并把1.5mm长的1.5→0.15mm taper做进pad1的自定义铜箔。
运行后会同步封装库和`ifa_geom.py`。
...
# ── V4.0 实板逐刀 VNA 定型尺寸（2026-09-04）────────────────────────
# ⚠️ 尺寸口径以**铜箔外沿**为准（2026-08-21 用户指定）。三种量法差 0.75–1.5mm，
# 之前就是因为口径不一致，量出来主臂 60、腿 8.2，对不上设计值。
#   主臂外沿长 = ARM_L + 2×HW      （两端各一个端帽）
#   腿包络高   = LEG_H + 2×HW
#
# 🔴 **2026-09-04 更正：本值曾是 49.000，那是记录错误，不要改回去。**
# V4.0 实板从原始 53.5mm 逐刀切到 **50.0mm 就停了，从来没到过 49.0mm**。
# 49.0 是当时未经核实的计划值，却被当成"实测定型"回灌进生成器/封装/PCB。
# 按下面这条逐刀曲线，49.0mm 会落到约 1107MHz——正是复测读到 1107.5MHz 的原因。
#
# 逐刀实测（同一块 V4.0 板，装盒+电池铺地）：
#   53.5mm → 995MHz          51.5mm → 1047.5MHz(装盒)
#   51.0mm → 1070–1072.5MHz  50.0mm → **1082.5MHz, SWR 1.09, 45+j0.5Ω**
# 两条独立同状态序列（光板 / 电池铺地）给出斜率 **24–25MHz/mm**，
# 取代文档旧口径的 24–25MHz/mm（那是 54→49 粗粒度前后对比，偏低 20%）。
#
# 另外两条同批实测，定尺寸时必须知道：
#   · 电池铺在 B 面稳定贡献 **+15MHz**（51.5 与 51.0 两个长度上分别 +15.0/+15.25）
#   · 电池位置本身值 **+9MHz**（挪到板中间靠下），是不动刀的调谐旋钮
# 屏幕引起的 1037.5/1125 双峰是**模态分裂**不是失谐（分裂宽度 86–87MHz
# 与天线长度无关，双峰中点 1081.25 与无屏时的 1082.5 重合），
# 只能靠 SD 卡座吸波片等结构手段解决，切铜治不了。
ARM_OUT = 50.000             # 主臂**铜箔外沿→外沿**；V4.0实板逐刀切短定型。
                             # 装盒+电池：1082.5MHz，SWR 1.09，45+j0.5Ω。
...
assert abs(ARM_L - 48.5) < 1e-9, "50.0mm外包络必须对应48.5mm中心线跨度"
...
s = ['(footprint "ANT_IFA_1090MHz"',
     ...
     f'\t(descr "1090MHz 板载 IFA（V4.0实板装盒VNA定型）。主臂中心线 {ARM_L}mm / 脚 {LEG_H}mm / '
