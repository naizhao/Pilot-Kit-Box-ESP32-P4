...
PCB="$D/kicad/expansion-board-v4.kicad_pcb"
...
if [ "$1" != "--keep" ]; then
  ...
  "$CLI" sch export netlist --format kicadxml -o /tmp/expansion.net.xml \
      "$D/kicad/expansion-board-v4.kicad_sch" >/dev/null 2>&1
...
cp "$D/kicad/expansion-board-v4.kicad_pro" "$KRT/kicad_files/w.kicad_pro"
