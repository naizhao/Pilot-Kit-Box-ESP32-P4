...
>
> **The machine assertion lives in `tools/gen_jlc_smt.py`** (`BUILD_VARIANT` + variant self-check);
> docs go stale, assertions don't — before changing placement, read the assertion first, don't just read this table.
...
Both are generated by `tools/gen_assembly_pdf.py` from the criteria in this table.
...
Before changing placement, run the generators rather than checking this table by hand:

```
python3 tools/gen_jlc_smt.py      # BUILD_VARIANT + variant self-check; wrong group errors out
python3 tools/gen_assembly_pdf.py # assembly map, includes class-coverage and public-output guards
```
