...
| 方案 | 贴片层 | 可选档位 | 5 片总价 |
|---|---|---|---|
| 带电源版（697 焊点 / 38 种） | 双面 | **只能标准型** | **¥649.91** |
| 不带电源版（598 焊点 / 32 种），**仍双面** | 双面 | 只能标准型 | ¥614.96 |
| 不带电源版（602 焊点 / 33 种），**单面** | 单面 | **经济型** | **¥267.1** |
| 同上走标准型 | 单面 | 标准型 | ¥420.16 |
...
1. **省钱全靠解锁经济型**——嘉立创的「双面贴片暂不支持经济型」是硬门槛。
   单面走标准型也才省 ¥195，走经济型才是 ¥348。
2. **不做到单面，分版本几乎白做**——614.96 vs 649.91 只差 ¥35，省的焊点费
   被固定的工程费+钢网费吃掉了。
...
## 4. ⚠️ 未决：不带电源版怎么给 RP2040 刷固件

不带电源版没有 J4，而 **J1 的 40 脚里没有任何可用的 USB**（见 §5 的查证）。
RP2040 的 BOOTSEL 是 USB-only，没有串口 bootloader，所以这是**阻塞项**。

三个方案，代价各不相同：

| | 不带电源版能否单面 | RP2040 怎么刷 | 代价 |
|---|---|---|---|
| **A** RP2040 USB 接 J3-27/25 (HS OTG) | ✅ | P4 做 host 代刷 | 永久占掉 H2 / 载板 USB-A，与 RTL-SDR 方案互斥；依赖微雪固件开放 host |
| **B** J4 挪到 F 面 | ✅ | 直接插电脑 | J4 逻辑上属于 B 面那套，但技术上零外部依赖 |
| **C** 维持现状 | ❌ 双面 | 直接插电脑 | 不带电源版只省 ¥35，等于白做 |
...
曾经计划给不带电源版补 5.1k 的 CC 下拉（USB-C device 要 Rd 才能枚举）。
**按 §1 的划分不需要**：不带电源版根本没有 J4，没有 USB 口，也就没有 CC。
带电源版的 CC 由 CH224K 内部的 Rd 承担。
