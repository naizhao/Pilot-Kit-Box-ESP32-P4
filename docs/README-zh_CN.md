...
- 目标开发板是 **Waveshare ESP32-P4-WIFI6-Touch-LCD-4.3**。P4 负责 UI 和存储；
  1090 MHz 解码在扩展板的 RP2040 上；C6 负责 BLE。
- ADS-B 接收：**v3/v4 扩展板**自带 1090 MHz 接收链、由 RP2040 解码。v1/v2 时代
  的 USB RTL-SDR dongle 路径已于 2026-09 移除（历史细节见 git 历史）：当前固件
  只支持扩展板接收链。
