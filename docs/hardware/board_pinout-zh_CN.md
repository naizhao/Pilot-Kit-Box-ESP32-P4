...
当前固件姿态变换对应 2026-08-03 重新焊装后的方位：芯片面仍朝屏幕，
但整块转接板绕板法线转了 180°。
...
固件应用 `q_body_fix = (0.7071068, 0, 0.7071068, 0)`。

⚠ 与旧值不同，当前的 R_chip→aircraft 是绕机体 Y 轴 90°，**不再是 180°**，
因此矩阵不自逆：`q_body_fix` 必须存它的**逆**（R_aircraft→chip），不能像
旧的 180° 那样直接存 R_chip→aircraft。

回归测试 `firmware/test/test_imu_mount.c` 从物理轴向映射独立推出四个姿态
下 BNO085 应输出的四元数（不复用被测宏），常量写错即报红：
...
若再次改动安装方位，必须同步修改变换、更新该测试里的 `chip_axes_in_aircraft()`
并重新验证姿态，不能只改文档。
