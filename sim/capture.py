...
SCENES: list[tuple[str, dict[str, str], str]] = [
    ...
    # 设置页新增的「罗盘校准」行（2026-08-03）。它是校准页的**唯一手动入口**：
    # 那一页此前只能自动弹（acc=0 满 10 s），而页内的「稍后再说」会关掉自动
    # 重弹的闸门，用户关过一次就再也进不去了。
    #
    # 两张都钉 PK_SIM_CAL_ACC=0：值框显示的是当前精度，0/3 既是最糟情况（真正
    # 需要校准的那一档），也是这一行存在的理由。中英各一张——英文
    # "COMPASS CAL" + "Quality  0 / 3" 是这一行的最长文案，要验它没压到标签、
    # 没顶出值框。
    ("ui-4.3-settings-cal", {"PK_SIM_PAGE": "settings",
                             "PK_SIM_SET_SCROLL": "400",
                             "PK_SIM_CAL_ACC": "0"},              "设置页「罗盘校准」行：值框显示当前精度 0/3，点进校准页"),
    ("ui-4.3-settings-cal-en", {"PK_SIM_PAGE": "settings",
                             "PK_SIM_SET_SCROLL": "400",
                             "PK_SIM_CAL_ACC": "0",
                             "PK_SIM_LANG": "en"},                "同上英文（本行最长文案：COMPASS CAL + Quality  0 / 3）"),

    # ── 磁力计校准向导 ─────────────────────────────────────────────
    #
    # 这是全设备**唯一一个用户点不进去**的页面：acc=0 连续 10 s 才自动弹。
    # 想在真机上看一眼，得先把 IMU 弄失准再等十秒；想把三档进度条都看齐，
    # 还得让 acc 依次走过 0/1/2——现场做不到，所以它必须有基线图。
    #
    # acc=0 那张是最糟情况：进度条一格没填（此时只有橙色边框在说话）、
    # 英文那版又是本页最长的文案（说明行 390 px、页脚 407 px）。
    # pressed 那张截的是「稍后再说」被按住的一帧——按钮唯一的"我收到了"
    # 信号，松手即消失，同样只有这里截得到。
    ("ui-4.3-cal",          {"PK_SIM_CAL": "1",
                             "PK_SIM_CAL_ACC": "0"},              "校准向导 acc=0：进度条空框（橙色边框=最差档）"),
    ("ui-4.3-cal-en",       {"PK_SIM_CAL": "1",
                             "PK_SIM_CAL_ACC": "0",
                             "PK_SIM_LANG": "en"},                "校准向导 acc=0（英文，本页最长文案）"),
    ("ui-4.3-cal-acc1",     {"PK_SIM_CAL": "1",
                             "PK_SIM_CAL_ACC": "1"},              "校准向导 acc=1：1/3 黄"),
    ("ui-4.3-cal-acc2",     {"PK_SIM_CAL": "1",
                             "PK_SIM_CAL_ACC": "2"},              "校准向导 acc=2：2/3 绿"),
    ("ui-4.3-cal-pressed",  {"PK_SIM_CAL": "1",
                             "PK_SIM_CAL_ACC": "0",
                             "PK_SIM_CAL_TAP": "700,436"},        "校准向导：「稍后再说」按住态（走真机同一条命中判定）"),

    # 顶栏的罗盘校准提示（PK_CAL_ADVICE_HINT：精度低但因为在飞 / 闸门关着而
    # 不抢页面的那一档）。它是这枚图标唯一的出场机会——真机上要么已经被拽进
    # 校准页（那就看不到顶栏了），要么得先按「稍后再说」再等一轮。
    #
    # 钉在 PFD 而不是别的页：四页顶栏共用 pk_ui_topbar_status_render，中段的
    # 左右边界又是与页面内容无关的固定公式，四页画出来是同一份。
    #
    # **必须配 NO_REC + NO_TEMP**，这不是为了好看：中段只有 200 px 硬预留
    # （BAR_MID_GUARANTEED_W），而模拟器默认把 REC 与 TEMP 一起点亮（满载，
    # 压顶栏溢出用），两项加起来 218 px 就已经超了，优先级最低的罗盘必然第一个
    # 被丢掉——那一档在下面 -crowded 那张里。这里关掉它俩摆的才是**真机常态**：
    # rec_active 至今没有数据源（pfd.c 的 TODO(P2-1) 恒为 false），temp_warn
    # 只在超温时为真，于是 SAT+ADSB+罗盘 = 173 px 装得下。
    ("ui-4.3-pfd-cal-hint", {"PK_SIM_CAL_HINT": "1",
                             "PK_SIM_NO_REC": "1",
                             "PK_SIM_NO_TEMP": "1"},              "顶栏罗盘提示（真机常态：未录制、不超温）：橙色常亮不闪，排在 ADSB 之后"),
    # 演示模式那张验的是另一件事：徽标常驻在顶栏最右端，中段整体左移之后
    # 这枚新图标不能被压在徽标下面。
    ("demo-4.3-pfd-cal-hint", {"PK_SIM_CAL_HINT": "1",
                             "PK_SIM_NO_REC": "1",
                             "PK_SIM_NO_TEMP": "1",
                             "PK_SIM_DEMO": "1"},                 "同上 + 演示模式：中段整体左移，罗盘图标不被 DEMO 徽标压住"),
    # 最挤的一档：中段五项全报（SAT/ADSB/REC/TEMP/罗盘 = 374 px）。断言的是
    # **它该被丢掉**——罗盘精度低不是故障（航向照出，只是可能不准），前面四项
    # 任何一项要位置它都得先让。这张与上面那张成对，缺了它就分不清"没画出来"
    # 是降级生效还是图标根本没接上。
    ("ui-4.3-pfd-cal-hint-crowded", {"PK_SIM_CAL_HINT": "1"},     "顶栏满载：罗盘作为最低优先级被降级丢弃，只剩 SAT/ADSB"),

    # ── 诊断页的「磁环境」行（阶段 C4）────────────────────────────────
    #
    # 干扰态是这枚判定唯一看得见的地方：那时自动弹页和顶栏图标**都被关掉**，
    # 设备表现为「什么都不提示」。所以两态必须成对截——只有干扰那张，分不清
    # 「正常态压根没画这一行」还是「画了，只是当时不该报警」。
    #
    # 详情页（DETAIL=0）是主要落点，键值两列都摆得开；总览卡片那张验的是另一
    # 件事：干扰时第二格由「航向 123」换成「磁干扰」，且状态色被压成琥珀
    # （不看当帧 accuracy——干扰的定义就是它在跳，那一拍可能正好停在 3）。
    #
    # 英文单独一张：值列最长文案是 "interference"（12 拉丁 = 180 px），中文
    # 「干扰」只有 44 px，中文那张验不出值列会不会被撑破。
    ("ui-4.3-diag-imu",     {"PK_SIM_PAGE": "diag",
                             "PK_SIM_DIAG_DETAIL": "0"},          "诊断 IMU 详情正常态：磁环境「正常」绿字，航向照给"),
    ("ui-4.3-diag-imu-jam", {"PK_SIM_PAGE": "diag",
                             "PK_SIM_DIAG_DETAIL": "0",
                             "PK_SIM_CAL_JAM": "1"},              "诊断 IMU 详情干扰态：磁环境「干扰」琥珀——设备闭嘴的原因在这里查"),
    ("ui-4.3-diag-imu-jam-en", {"PK_SIM_PAGE": "diag",
                             "PK_SIM_DIAG_DETAIL": "0",
                             "PK_SIM_CAL_JAM": "1",
                             "PK_SIM_LANG": "en"},                "同上英文（值列最长文案 interference，验不撑破值列）"),
    ("ui-4.3-diag-jam",     {"PK_SIM_PAGE": "diag",
                             "PK_SIM_DIAG_OK": "1",
                             "PK_SIM_CAL_JAM": "1"},              "诊断总览干扰态：IMU 卡第二格由航向换成「磁干扰」，状态色压成琥珀"),
