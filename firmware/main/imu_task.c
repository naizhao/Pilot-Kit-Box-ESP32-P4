...
/* 罗盘航向固定修正（度）。
 *
 * 2026-09-11 用户实机四点标定（V4 竖放，各指向已知朝向读屏上 yaw）：
 *   加 +90 修正后：东90→170(+80)、南180→245(+65)、西270→345(+75)
 * 三点误差均值 +73°、散差 ±8° → 真实固定偏差 ≈ -17°（接近磁偏/硬铁偏置，
 * 不是 90/180 的整倍数，排除安装轴错误）。故取 +17 居中；残余 ±8° 散差
 * 是磁场干扰，需从磁源/位置解决（见日志 |B|，地球 ~45uT）。
 *
 * 待办：这是固定补偿，不是真校准；磁校准能落盘后应改回 0 并靠 DCD。 */
#define IMU_YAW_OFFSET_DEG  17.0f
