...
void pk_boot_splash_render(uint16_t *fb)
{
    ...
    /*
     * 屏底这一带：**免责声明与演示横幅二选一，不并存**。
     *
     * 为什么不能都画：进度版面下三行构建信息收在 y≈423（实测，屏高 480），
     * 屏底只剩 57 px；横幅 40 px + 免责一行 26 px = 66 px，放不下。硬塞的
     * 后果是免责那行被红条压掉半截，比不画更糟。
     *
     * 为什么演示横幅优先：它说的「数据是模拟的」比「仅供辅助参考」**严格更强**，
     * 而演示模式是台面测试模式、不是飞行模式。完整条款常驻在「关于」页页脚，
     * 任何时候都在，不依赖这一屏。
     */
    if (!pk_demo_enabled()) {
        /*
         * 免责一行。**只显示、不拦截**——开机流程不为它停一帧，也没有确认按钮。
         * 座舱里一次多余点击的代价远高于电脑上；每次上电都弹框，用户学会的不是
         * "这设备只能辅助"，而是"闭着眼睛点掉那个框"。
         */
        const char *msg = pk_i18n_text(PK_TR_DISCLAIMER_SPLASH);
        /* 中文必须走 pk_aa_text_width：按 strlen×cell 算的是字节数，整行会
         * 左偏三分之一——下面那条横幅正是栽在这上面。 */
        const int w = pk_aa_text_width(msg, PK_AA_S);
        pk_aa_puts(fb, PK_DISPLAY_W, PK_DISPLAY_H,
                   (PK_DISPLAY_W - w) / 2, PK_DISPLAY_H - PK_AA_S_H - 10,
                   msg, VERSION_COLOR, PK_AA_S);
    } else {
        /*
         * 演示模式横幅。
         *
