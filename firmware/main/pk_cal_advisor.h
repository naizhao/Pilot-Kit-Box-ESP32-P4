...
/* 低精度（acc<EXIT_ACCURACY，即 0 或 1）连续累计到这么久，才认为「确实需要
 * 校准」。
 * 旧值是 10 s（ui_state.c 的 UI_CAL_WIZARD_ENTER_MS），上一轮拉长到 20 s，
 * 这一轮保留 20 s 作为「收敛过又退化」的快通道窗口。
 *
 * 为什么旧实现只敢 10 s：它没有别的护栏，短窗口能让「真没校准」早点被发现；
 * 现在相位门控 + 重新武装滞回 + 干扰识别 + 静止门控 + 冷启动宽限五道都上了，
 * 误弹的路径已经堵死，窗口按「设备真的在动、精度仍然低」来定——20 s 是「明显
 * 长过典型收敛时间、又不至于让真需要校准的人干等」的折中。 */
#define PK_CAL_ENTER_MS 20000u

/* 冷启动宽限：本次开机以来 acc **从未**到过 EXIT_ACCURACY 时，把 ENTER 窗口
 * 临时拉长到这个值。实测 BNO085 冷启动自然收敛需要 29~30 s，而 ENTER 窗口是
 * 20 s——不宽限的话弹窗比自己收敛好只早 6.6 s，纯属添乱。120 s 留 4 倍余量：
 * 既盖住「这次冷启动恰好慢一点」，又不至于把真没校准又一直没动的盒子晾太久。
 * 一旦 acc 上到过 EXIT_ACCURACY（哪怕一帧），本次开机就再不享受宽限——
 * 之后的退化走 20 s 快通道，因为「校准过又掉下来」说明换了环境，该早点提醒。 */
#define PK_CAL_COLDSTART_GRACE_MS 120000u
...
/* ------------------------------------------------------------ 静止门控
 *
 * 磁力计校准**物理上需要设备转动**：画 8 字的本质是让传感器采到不同朝向的
 * 磁场样本。盒子放桌上不动时提示「请画 8 字」是白提——用户不动它就永远好不了，
 * 动了才有意义。所以静止时只给 HINT（顶栏图标：用户得知道精度不够，看到图标
 * 才会想起来拿起来转一转），不抢页面；只有检测到「在动」且精度仍然低，才弹页
 * 教画 8 字。
 *
 * 「静止」怎么判：用 pk_vib.c 的加速度模长 RMS（pk_imu_sample_t.vib_level）。
 * 真机实测对照组（1 Hz 日志里抓的）：
 *   静止放桌面        vib = 4   （49/55 拍是 4，偶尔 5）
 *   手指轻微触碰      vib = 13
 *   拿起来转动画 8 字  vib = 125 ~ 158
 *   窗口未满（开机头两拍）vib = 0  ← 0 是「不可用」不是「零振动」
 *
 * 阈值定 20：静止(4)、轻触(13) 都在阈值下，真正的转动(125+) 远在阈值上，
 * 中间隔着一个数量级。**轻触(13) 刻意算作静止**——那种幅度的晃动不足以让
 * 磁力计采到不同朝向的样本，提示了也是白提。 */
#define PK_CAL_VIB_STILL_THRESH 20u
...
typedef struct {
    /* 低精度（acc < EXIT_ACCURACY，即 0 或 1）连续段的起点。*_active=false 表示
     * 当前没有这样的连续段。用独立的 bool 而不是「0 表示没有」的哨兵：now_ms 是
     * uint32 毫秒，0 本身是合法时刻（回绕后必然出现），拿它当哨兵会在回绕那一
     * 拍误判。 */
    bool     low_active;
    ...
    /* 本次开机以来 acc 是否到过 EXIT_ACCURACY。用来区分两种低精度：
     *   - 从未到过（冷启动还没收敛）→ ENTER 窗口临时拉长到 COLDSTART_GRACE_MS，
     *     实测冷启动自然收敛要 29~30 s，20 s 窗口会在它自己好之前 6.6 s 就弹窗；
     *   - 到过又掉下来（换环境/退化）→ 走 20 s 快通道。
     * 一旦置位，本次开机再不享受宽限（没有「再冷启动」这回事）。 */
    bool     ever_converged;

    /* 校准页是用户主动打开的（设置页那一行）还是系统自动弹出的。
     * D3：用户主动点进来就是要校准，系统不能凭一个 acc 读数判定「没事可做」
     * 然后把页面收走。该标志为真时 should_exit_wizard() 恒假，页面只能由用户
     * 自己离开（「稍后再说」/ FAB→导航网格）。dismiss / 自动进入路径都会清位。 */
    bool     user_opened;
} pk_cal_advisor_t;
 ...
 *
 * vib_level 取自 pk_imu_sample_t.vib_level（pk_vib.c 的加速度模长 RMS）。
 * **vib_level==0 是「不可用」不是「零振动」**（pk_vib.h 明确警告过）。不可用时
 * 的静止门控策略见实现的注释。 */
 ...
 * D3：同时清掉 user_opened——用户自己离开了这一页，之后若系统再次自动弹出，
 * 那次弹出仍受正常的自动退出管辖。不清位的话，一次主动进入会永久禁用所有
 * 后续自动弹出的退出逻辑。 */
 ...
 * D3：置位 user_opened——用户主动点进来的页面永不自动退出（见
 * should_exit_wizard 的注释），只能由用户自己离开。 */
 ...
 *
 * D3：若校准页是**用户主动打开**的（user_opened 为真），本函数恒返回 false ——
 * 用户主动点进来就是要校准（重新校准/验证/画 8 字顶精度都正当），系统不能凭
 * 一个 acc 读数判定「没事可做」然后把页面收走。页面只能由用户自己离开
 * （「稍后再说」/ FAB→导航网格）。只有系统自动弹出的页面才会在 acc 稳定达标
 * EXIT_MS 后自动退回。
 *
 * 取值口径与 is_jammed 一致：都是「截至上一次 update」的结论，所以是不带
 * now_ms 的 const getter。 */
