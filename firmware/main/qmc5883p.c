...
/* ── 初始化序列（数据表，host 可测）────────────────────────────────────
 * 顺序与 29H=0x06 = PDF §7.2 Continuous Mode Setup Example 逐条（§7.1 Normal /
 * §7.3 Self-test 两个示例同样以 29H=0x06 开头，p.12/18）；0BH/0AH 取值为
 * Table 17/18 字段级自选（±2G/10 Hz，出处见各常量行内引用）——§7.2 示例自身
 * 写的是 0BH=0x08（±8G）/0AH=0xC3（200Hz），与这里的交付值不同。
 * 0BH 先于 0AH：模式位最后落笔，器件带着定好的量程进入连续测量。
 *
