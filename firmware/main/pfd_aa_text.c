 ...
 *
 * cell **宽**是整一个 em，因此明显宽于同档拉丁（M 档 22 vs 15）。这不是留白
 * 过多：Noto Sans SC 的汉字字面就是照满 em 画的，窄一个像素就切掉外框笔画。
 * 早先按 round(拉丁pt × 0.81) 取宽，254 字里有 190~230 个被左右各削 1~2 px，
 * 加上 CJK 侧还乘了 0.92 的 pointsize 补偿，中文实测比英文小 15~20%——真机
 * 反馈的「中文比英文偏小 1~2 号」就是这么来的。详见生成脚本头部的实测表。
 *
 * XL 档没有中文：它只服务 PFD 当前值，纯数字带正负号。XS 档有——那里要画
 * 方向箭头 ↑↓←→，它们和汉字走同一条非 ASCII 查表路径。 */
typedef struct {
    const uint8_t *bitmap;
    int            cell_w;
    int            cell_h;
