 ...
 *
 * 产品决策（2026-08-02 评审）：FAB 就是浮在内容之上的，全局 UI 都这么设计，
 ...
 * 右对齐到旧位置、线画在新位置，字被线穿过去。缩略图上看不出来，放大后
 * 才发现。
 *
 ...
 *
 * 约定（阶段 4b 地面 CPR 接入时立的）：地面目标（on_ground）恒无气压高度
 * （aircraft_state.c 的 metype 5-8 分支从不设 have_altitude），所以
 * pk_traffic_rel_calc() 算出来的 rel_alt_valid 对地面目标恒为 false——
 * 这条函数的判据不用改，天然就不会给滑行中的飞机标红。**将来若给地面目
 * 标接上任何形式的高度（哪怕只是机场标高兜底），也不得让它进入这条威胁
 * 判据**：滑行道上一堆飞机同高度、近距离扎堆是常态，接上高度只会让整屏
 * 红光——飞行员会开始忽略所有告警，这就是「狼来了」。
 */
 ...
 * 取消本次触摸：只丢状态，**不执行**动作。
 *
 * 与 touch_up 的区别：cancel 是"这一下当没发生过"，touch_up 会把之前的按下
 * 当成一次完整点击提交出去。
 *
 * 现在谁在用（2026-08-02 普查）：只有模拟器 sim/main.c 的 FAB 放行自检
 * （"各页面必须把 FAB 头上的触摸放行"那一段）——它要在两个吸附侧各试一次
 * 按下，中间必须把列表的按压状态清干净又不能触发点击，只有 cancel 合适。
