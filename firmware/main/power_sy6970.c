...
/* ── 充电中电量虚高：实测记录与尚未解决的部分（2026-09-12/13）─────────
 *
 * 电量准确度目前的**主要误差源**，这里把已经测到的和还没解决的都记下来，
 * 免得下一个人重做一遍实验。
 *
 * 【已证实】停充维持压降 = 0。
 *   终止后仍插着电 4164 mV，拔掉线还是 4164 mV，一模一样。SY6970 有独立
 *   BATFET 与 SYS 轨，不像 ETA6098 那样被充电器把 BAT 节点抬起来（那颗当年
 *   实测 96 mV）。**所以 SY6970 路径不需要 ETA 式的 HOLD 档补偿**，
 *   power_eta6098.c 里 BATT_HOLD_DROP_MV=96 那一档不要照搬过来。
 *
 * 【已测到，但不可直接用】CV 末期压降 ≈ 80 mV @ ICHG≈270 mA。
 *   换算成 R≈296 mΩ，但这个数**不能当欧姆内阻用**，两个原因：
 *     1. 4244 与 4164 都是 20 mV LSB 的量化值，80 mV 带 ±25% 不确定度；
 *     2. 采样间隔 10 s，测到的是「I×R 瞬时压降 + 电池极化消退」的混合量，
 *        真正的欧姆内阻要靠电流切断瞬间的毫秒级跳变才分得出来。
 *   反证：若真按 296 mΩ 线性外推到 CC 段（2 A），压降会算成 600 mV，
 *   意味着 CC 段只能充到约 30% SoC——与锂电充电的实际形态明显不符。
 *   **结论：简单的「压降 = ICHG × R」单系数模型在本板不成立**，要么分段
 *   标定（同 ETA 的 CC/HOLD 两档），要么只在 CV 段补偿。
 *
 * 【当前做法是权宜之计】见下面那段 charging 封顶 99% 的注释。它掩盖顶部
 *   矛盾，但中间段（60%~90%）的虚高**依然存在且没人看得见**。
 *
 * 【下一步】放电曲线标定：满电拔线后连续记录「时间↔BATV」直到放空，结合
 *   5000 mAh 标称容量反推这块电池的真实电压–SoC 曲线，替换 pk_batt_model
 *   里那条通用曲线（那五个拐点从未用本机电池验证过，是中段误差的另一个
 *   来源）。同一份数据还能给出平均放电电流——SY6970 的 ICHGR 只测充电方向
 *   （[DS] p.24-25），放电电流在本硬件上测不到。
 */
