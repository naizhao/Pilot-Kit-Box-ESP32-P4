...
int main(void)
{
    ...
    while (true) {
        ...
        if (c == 'T') {
            ...
            /* CC1312R 代刷模式（cJTAG 位脉冲）：暂停 1090 解码、
             * 独占 SUBG_TMSC/TCKC/RESET → 读 IDCODE → 流式收镜像 →
             * 擦/写/校验 → RESET → 恢复 SPI master。
            ...
            if (cjtag_cdc_enter()) {
                ...
                printf("FLASH-MODE FAIL (no JTAG response — check CC1312R power/clock)\n");
            }
