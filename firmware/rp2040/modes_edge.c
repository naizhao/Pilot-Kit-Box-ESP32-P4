...
#define QUS_CHIP       8     /* 2.0µs/chip */
#define QUS_B1_AT      0     /* bit1 脉冲在 chip +0.0µs */
#define QUS_B0_AT      4     /* bit0 脉冲在 chip +1.0µs */
#define QUS_BURST_GAP  20    /* >5µs 无沿 = burst 结束 */

static uint32_t ticks_to_qus(uint32_t ticks, uint32_t tick_hz)
{
    /* 四舍五入：沿在被捕获时已量化到最近 tick，反变换同样取最近 qus。
     * 逐段截断在 62.5MHz（1 qus = 15.625 tick）下会让每个 4/12 qus 间隔
     * 固定缩 1 qus，112 chip 累积漂移远超 ±1 qus 容差。uint64 防溢出。 */
...
static void burst_emit(modes_edge_t *m)
{
    ...
    /* 组帧：逐 chip 判位；chip 窗口无脉冲或有歧义 → 帧作废。
     * bit1 窗 = chip±1 qus；bit0 窗 = chip+4±1 qus；两窗之间（chip+1..chip+3
     * 开区间）视为歧义。窗前的沿按噪声跳过。t[] 本就相对 e0，数据块
     * 自 e0 +8µs 起，故 rel 直接与 chip 比较，沿下标从 3（首数据脉冲）起。 */
    uint8_t frame[14] = {0};
    int got = 0, ei = 3;
    const uint32_t data0 = QUS_DATA_OFF;
    for (int k = 0; k < 112; k++) {
        uint32_t chip = data0 + (uint32_t)(QUS_CHIP * k);
        int bit = -1;
        while (ei < m->burst_n) {
            uint32_t rel = t[ei];
            if (rel + QUS_TOL < chip) { ei++; continue; }     /* 窗前噪声沿 */
            if (rel >= chip - QUS_TOL && rel <= chip + QUS_TOL)          bit = 1;
            else if (rel >= chip + QUS_B0_AT - QUS_TOL &&
                     rel <= chip + QUS_B0_AT + QUS_TOL)                  bit = 0;
            if (bit >= 0) ei++;                               /* 本沿已被消费 */
            break;                                            /* 其余情形：本 chip 无脉冲 */
        }
        if (bit < 0) break;
