...
1. **一次事务 = CSN 低电平期间的定长 512 B 交换；应答延迟一拍（deferred-
   ...
   之间 CSN 必须回高并保持 ≥ 1 µs（CSN 上升沿 = 事务边界）。**master
   侧另须软件保证相邻事务间隔 ≥ 1 ms**（R14 修订：slave 的 CSN↑ 后
   处理需要数百 µs，见修订记录；500 ms HELLO 重试与 1 Hz PING 天然
   满足）。
...
- **3.5 seq 语义（延迟应答模型，§2.2；单一计数域）**：应答类帧（HELLO
  ...
  **ERROR 的 seq 按 code 分两类**：0x01–0x03（版本/状态/RF_CONFIG，回应
  具体命令）回显命令 seq；0x04（溢出通报）/0x05（内部错误）为**异步
...
QUEUE_FULL 帧**交付完成**之事务的 CSN 上升沿清——装载不清（装载与交付相隔
...
drain 中队列重新填满会再次置位并再次交付。**防抢占流的真实保证**：
① **清除与下一次装载发生在同一 CSN 上升沿（原子）**——标志不存在
「已重新置位却未随交付清除」的叠加残留，连续两个槽不会都装 QUEUE_FULL
而中间无交付；② 分片取毕期间规则 1（chunk 链延续）优先于规则 4，在途
报文的取片流水不会被重复的 QUEUE_FULL 打断。重填置位可以发生在交付后
1 个事务窗口内（队头已装载未弹出、队列仍满时新到达）——此时下一次
装载仍是 QUEUE_FULL，属正常通报而非抢占；队列每被取空一次即恢复普通
事件流动。且每次交付都是事件事务、使 §7.2 停滞计数保持复位，
...
| R12（本提交） | 审计 round 9 Ruling 1：**清除时点统一为交付完成**——携带该源帧之事务的 CSN 上升沿（装载不清；装载与交付相隔一事务，IRQ 期间保持高，关闭装载即清导致的 IRQ 提前落低/QUEUE_FULL 抢占流）。§1 升级为「IRQ 置位 ⟺ 存在尚未完成 MISO 交付的挂起源」并成为各源清除时点的唯一规范出处（四源逐条）；§2.3 规则 4 移除装载伴随清除语义；§4.5/§4 表/§7.2（装载不是事务）同步；B34/B35 走查补时间点 | §1、§2.3、§4 表、§4.5、§7.2、B.3 |
| R14（本提交） | 审计 WP-E-1 P1-4：**master 事务间隔合同**——线级 CSN 高 ≥1 µs 不足以覆盖 slave 的 CSN↑ 后处理（512 B 帧校验 + 装载缓冲构造，48 MHz 下最坏数百 µs）；master 必须**软件保证相邻事务间隔 ≥1 ms**（骨架保守值，台架实测后可压缩并修订本条）。这是 §6.7 之外的 master 侧行为合同；等待 HELLO 的 500 ms 重试与 1 Hz PING 天然满足 | §2.1 注、§6.7 引 |
