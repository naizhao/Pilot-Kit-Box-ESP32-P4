...
int main(void){
    ...
    r = pk_traffic_rel_calc(true,0,0, 0,0,1000, true,1,0, true,2000, true,0);
    ...
    /* 朝北，目标正东 → rel +90 */
    r = pk_traffic_rel_calc(true,0,0, 0,0,1000, true,0,1, true,0, true,0);
    chk("E rel+90",r.rel_bearing,90,0.5);
    /* 机头朝东(hdg90)，目标正北 → rel -90 (在我左) */
    r = pk_traffic_rel_calc(true,0,0, 90,0,1000, true,1,0, true,0, true,0);
    chk("hdg90 N rel-90",r.rel_bearing,-90,0.5);
    /* 磁偏角 +6(东偏)：目标真北方位0 → 磁方位 -6→354 */
    r = pk_traffic_rel_calc(true,0,0, 0,6,1000, true,1,0, true,0, true,0);
    chk("magvar abs354",r.abs_bearing,354,0.5);
    /* 本机无气压高度 → rel_alt_valid false */
    r = pk_traffic_rel_calc(true,0,0, 0,0,PK_ALT_UNAVAIL, true,1,0, true,5000, true,0);
    chk("relalt invalid",r.rel_alt_valid?1:0,0,0);
    /* 目标无位置 → valid false */
    r = pk_traffic_rel_calc(true,0,0, 0,0,1000, false,1,0, true,0, true,0);
